package com.sleepyzzz.handlewificamera.custom;
...
public class PlayerView extends FrameLayout implements IVideoPlayer {
    ...
    private static final int mCurrentSize = SURFACE_BEST_FIT;
    ...
    /**
     * ClassName: OnChangeListener
     * Description: TODO-接口监听缓冲进度
     * author SleepyzzZ
     * date 2016-1-15 上午10:02:18
     */
    ...
    private final SurfaceHolder.Callback mSurfaceCallback = new SurfaceHolder.Callback() {
        ...
        public void surfaceChanged(SurfaceHolder holder, int format, int width,
            ...
            if (format == PixelFormat.RGBX_8888)
                Logger.t(TAG).d("Pixel format is RGBX_8888");
            else if (format == PixelFormat.RGB_565)
                Logger.t(TAG).d("Pixel format is RGB_565");
            else if (format == ImageFormat.YV12)
                Logger.t(TAG).d("Pixel format is YV12");
            else
                Logger.t(TAG).d("Pixel format is other/unknown");
    ...
    /**
     * ClassName: VideoPlayerHandler
     * Description: TODO-监听视频流播放状态
     * author SleepyzzZ
     * date 2016-1-15 上午10:09:59
     */
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.getData().getInt("event")) {
                ...
                case EventHandler.MediaParsedChanged:
                    Logger.t(TAG).d("MediaParsedChanged");
                    break;

                case EventHandler.MediaPlayerPlaying:
                    Logger.t(TAG).d("MediaPlayerPlaying");
                ...
                case EventHandler.MediaPlayerPaused:
                    Logger.t(TAG).d("MediaPlayerPaused");
                    break;

                case EventHandler.MediaPlayerStopped:
                    Logger.t(TAG).d("MediaPlayerStopped");
                    break;

                case EventHandler.MediaPlayerEndReached:
                    Logger.t(TAG).d("MediaPlayerEndReached");
                    if (playerView.mOnChangeListener != null) {
                ...
                case EventHandler.MediaPlayerEncounteredError:
                    Logger.t(TAG).d("MediaPlayerEncounteredError");
                    if (playerView.mOnChangeListener != null) {
                ...
                case EventHandler.HardwareAccelerationError:
                    Logger.t(TAG).d("HardwareAccelerationError");
                    if (playerView.mOnChangeListener != null
                ...
                case EventHandler.MediaPlayerBuffering:
                    Logger.t(TAG).d("MediaPlayerBuffering");
                    if (playerView.mOnChangeListener != null) {
                ...
                default:
                    Logger.t(TAG).d(String.format("Event not handled (0x%x)", msg
    ...
    private void init() {
        ...
        // Subtitles view
        // mSubtitlesSurface = (SurfaceView)
        // findViewById(R.id.subtitles_surface);
    ...
    /**
     * Title: changeSurfaceSize
     * Description: TODO-changeSurfaceSize
     * param 设定文件
     * return void 返回类型
     * throws
     */
    public void changeSurfaceSize() {
        ...
        if (dw * dh == 0 || mVideoWidth * mVideoHeight == 0) {
            Logger.t(TAG).e("Invalid surface size");
            return;
        }
